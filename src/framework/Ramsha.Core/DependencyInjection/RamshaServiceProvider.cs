...
internal class RamshaServiceProvider : IRamshaServiceProvider, IEnumerable<ServiceDescriptor>
{
    private readonly IServiceCollection _services;
    private readonly ILogger<RamshaServiceProvider> _logger;
    ...
    private static readonly Type[] _selfTypes = [typeof(IRamshaServiceProvider), typeof(IServiceProvider), typeof(IKeyedServiceProvider), typeof(IServiceProviderIsService), typeof(IServiceProviderIsKeyedService), typeof(ISupportRequiredService), typeof(IServiceScopeFactory)];

    public RamshaServiceProvider(IServiceCollection services, ServiceProviderOptions? options = null, IServiceProviderResolver? resolver = null)
    {
        ArgumentNullException.ThrowIfNull(services, nameof(services));
        _services = services;
        _serviceProvider = services.BuildServiceProvider(options ?? RamshaServiceProviderFactory.DefaultServiceProviderOptions);
        _serviceProvider = (_serviceProvider.CreateScope().ServiceProvider as IKeyedServiceProvider)!;
        _logger = _serviceProvider.GetRequiredService<ILogger<RamshaServiceProvider>>();
